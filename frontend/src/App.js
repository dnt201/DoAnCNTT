          ...
          listCategories.map((category) => (
            <React.Fragment key={category.id}>
              <Route
                key={category.id}
                path={"/" + category.path}
                element={<ListCategoryPage category={category} />}
              />
              {category.children &&
                category.children.map((categoryChild) => (
                  <Route
                    key={categoryChild.id}
                    path={"/" + category.path + "/" + categoryChild.path + "/"}
                    element={
                      <ListProductPage productType={categoryChild.name} />
                    }
                  />
                ))}
              {category.children &&
                category.children.map((categoryChild) => (
                  <Route
                    key={categoryChild.id}
                    path={
                      "/" + category.path + "/" + categoryChild.path + "/:id"
                    }
                    element={
                      <DetailProductPage productType={categoryChild.name} />
                    }
                  />
                ))}
            </React.Fragment>
