    ...
    <div className="admin-list-product">
      {showAddProductPopup && (
        <AddProduct
          ...
          handleClose={addHandler}
        />
      )}
      {showDetailProductPopup && (
        <ChangeProduct id={id} handleClose={detailHandler} />
      )}
        ...
        <div>
          <div className="lazy-header-css">
            <div className="m-r-32px flex-1">
              <span className="f-s-36px">
                <b>List product {props.product.name}</b>
              </span>
              {count && total ? (
                <i>
                  ({count} of {total})
                ...
                <i>(Chưa có sản phẩm bạn ơi!)</i>
              )}
            </div>
            <button
              className="add-product-btn"
              onClick={() => addHandler(props.product.name)}
            >
              {" "}
              Thêm sản phẩm{" "}
              <FontAwesomeIcon className="icon icon-change" icon={faPlus} />
            </button>
          </div>
            ...
            {products &&
              products.map((product) => (
                <div key={product.id} className="product-item">
                  <span className="item-name flex-4">
                    <b>{product.name}</b>
                  </span>
                  <img
                    className="item-image flex-4"
                    src={product.image}
                    alt="tôi cũng không biết làm gì khúc này cả"
                  />
                  <span className="item-price flex-2">
                  ...
                  <button
                    className="btn btn-change flex-1"
          ...
          {count && total ? (
            ...
            <h3 className="t-a-center">(Opps!)</h3>
          )}
        </div>
      ) : (
        <h3 className="center-screen color-red">
          <i> Chọn loại sản phẩm (Nếu chưa có vui lòng tạo sản phẩm)</i>
        </h3>
      )}
    </div>
  );
