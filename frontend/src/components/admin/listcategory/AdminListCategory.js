...
const AdminListCategory = () => {
    ...
    const { loading: loadingDelete, success: successDelete, error: errorDelete } = deleteCategoryState
    ...
    return (
        <div className="list-category-container">
            ...
            {isUpdate && <UpdateCategory handleClose={handleUpdate} AddCategory id={id} name={name} image={image} type={type} />}
            ...
            {successDelete && <h4 className="t-a-center"><i className="color-red"><b> Xóa thành công {errorDelete}</b></i></h4>}
            {errorDelete && <h4 className="t-a-center"><i className="color-red"><b> Something went wrong! {errorDelete}</b></i></h4>}
            {loadingDelete && <div className="loader-center-screen"><Loader /></div>}
                ...
                <div className="wrap">
                    <div className="flex-row align-item-center jt-ct-sb" key={category.id}>
                        ...
                        <div className="flex-1 flex-row jt-ct-sa">
                            ...
                            <button className="btn-delete" onClick={() => handleDelete(category.id)}>Delete <FontAwesomeIcon icon={faTrashAlt}/></button>
                        </div>
                    </div>
                    ...
                    {category.children && category.children.map((child) => (
                        <div className="flex-row m-l-32px align-item-center m-t-8px" key={child.id}>
                            ...
                            <div className="flex-1 flex-row jt-ct-sa">
                                ...
                                <button className="btn-delete" onClick={() => handleDelete(child.id)}>Delete <FontAwesomeIcon icon={faTrashAlt}/></button>
                            </div>
                        </div>
                    ))}
                </div>
